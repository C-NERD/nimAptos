...
# third party imports
from pkg / libsodium / sodium import crypto_sign_seed_keypair, crypto_sign_ed25519_sk_to_seed, 
...
template sign(prvkey, data : string, code : untyped) : untyped = 
    ...
    let 
        ...
        signature {.inject.} = toLowerAscii(toHex(signedMsg[0..<crypto_sign_bytes()]))
