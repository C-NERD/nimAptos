...
proc signMsg*(account : RefMultiSigAccount, msg : string) : seq[tuple[ownerpos : int, signature : string]] = 
    
    for pos in 0..<len(account.accounts):

        result.add (pos, signMsg(account.accounts[pos], msg))

proc verifySignature*(account : RefMultiSigAccount, signatures : seq[tuple[ownerpos : int, signature : string]], msg : string) : bool = 
...
template multiSignTransaction() =
    ...
    for pos in 0..<acctLen:
        ...
        assert verifySignature(account.accounts[pos], $signature, submission), "cannot verify signature for " & $pubkey
        bitPos.add pos
...
proc multiSignTransaction*[T : TransactionPayload](account : RefMultiSigAccount, client : AptosClient, transaction : RawTransaction[T], encodedTxn : string = "") : Future[SignedTransaction[T]] {.async.} =
...
proc multiSignTransaction*[T : TransactionPayload](account : RefMultiSigAccount, transaction : RawTransaction[T], encodedTxn : string = "") : SignedTransaction[T] =
...
proc createMultiSigAccount*(client : AptosClient, owners : seq[RefAptosAccount], threshold : uint64) : Future[RefMultiSigAccount] {.async.} =
...
## MultiSig move procs
proc createMultiSigTransaction*(account : RefMultiSigAccount, payload : HexString) : EntryFunctionPayload =
...
proc voteOnTransaction*(account : RefMultiSigAccount, sequenceNumber : uint64, vote : Vote) : EntryFunctionPayload =
...
proc removeRejectedTransactions*(account : RefMultiSigAccount, finalSequenceNumber : uint64) : EntryFunctionPayload =
...
proc getPendingTransactions*(account : RefMultiSigAccount, client : AptosClient) : Future[seq[MultiSigTransaction]] {.async.} =
